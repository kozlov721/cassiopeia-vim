" -----------------------------------------------------------------------------
" Name:         Cassiopeia
" Description:  Clean dark theme, resembling the stars on night sky.
" Author:       Martin Kozlovský <kozlovsky.m7@gmail.com>
" Website:      https://github.com/xkozlov1/cassiopeia.vim/
" License:      MIT
" -----------------------------------------------------------------------------
...
let g:colors_name = 'cassiopeia'
" }}}
" Configuration: {{{
let s:configuration = {}
let s:configuration.style = get(g:, 'cassiopeia_style', 'night')
let s:configuration.transparent_background = get(g:, 'cassiopeia_transparent_background', 0)
let s:configuration.menu_selection_background = get(g:, 'cassiopeia_menu_selection_background', 'green')
let s:configuration.disable_italic_comment = get(g:, 'cassiopeia_disable_italic_comment', 0)
let s:configuration.enable_italic = get(g:, 'cassiopeia_enable_italic', 0)
let s:configuration.cursor = get(g:, 'cassiopeia_cursor', 'auto')
let s:configuration.current_word = get(g:, 'cassiopeia_current_word', get(g:, 'cassiopeia_transparent_background', 0) == 0 ? 'grey background' : 'bold')
" }}}
" Palette: {{{
"
let s:palette = {
